...
module tb_apb_uart_rx ();
    ...
    task run_test_sequence;
        ...
        begin
            ...
            check_output(4, {4'b0, data_size}, {test_name, " data_size"});
            check_output(2, bit_period[7:0], {test_name, " bit_period_0_size"});
            check_output(3, {2'b0, bit_period[13:8]}, {test_name, " bit_period_1_size"});
            
            stream_data(test_data, 200ns);
            
            check_output(0, 8'h01, {test_name , " data_status"});    
            check_output(1, 8'h00, {test_name, " error_status"});
            check_output(6, test_data[test_data.size()-1], {test_name, " buffer val"});
        end
    endtask
    ...
    initial begin
        ...
        configure_design(14'd10, 4'd8); //bit period = 10; data_size = 8

        test_name = "read data size";
        check_output(4, 8, test_name);

        test_name = "read bit period";
        check_output(2, 10, test_name);
        check_output(3, 0, test_name);

        test_name = "stream 8 bit";
        streaming(8'b10101010);
        test_name = "stream 8 bit data status";
        check_output(0, 1, test_name);
        test_name = "stream 8 bit error";
        check_output(1, 0, test_name);
